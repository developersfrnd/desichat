...
function EmailVerification() {
    ...
    const submitEventHandler = (data) => {
        setinProgress(true);
        usersModel.getVerificationCode(data)
            .then((res) => {
                setEmail(data.email)
                setTokenSend(true)
            })
					...
					<div className="row">
                        ...
                        (                           
							...
							<form className="contact-form" method="post" onSubmit={ handleSubmit(submitEventHandler) }>
                                <h2 className="big margin_0">Email Verification</h2>
							    <h2 className="muellerhoff topmargin_5 bottommargin_50 highlight">Get the code to verify your email</h2>
